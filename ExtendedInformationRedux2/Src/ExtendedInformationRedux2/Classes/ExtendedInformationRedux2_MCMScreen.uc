...
class ExtendedInformationRedux2_MCMScreen extends Object config(ExtendedInformationRedux2);

`include(ExtendedInformationRedux2\Src\ModConfigMenuAPI\MCM_API_Includes.uci)
...
simulated function ClientModCallback(MCM_API_Instance ConfigAPI, int GameMode)
{
	...
	for (i=LayoutArray.Length;i<class'ExtendedInformationRedux2_UITacticalHUD_ShotHUD'.default.Offsets.Length;i++)
	{
...
simulated function SaveButtonClicked(MCM_API_SettingsPage Page)
{
	...
	if (TacticalHUD!=none)
	{
		ExtendedInformationRedux2_UITacticalHUD_ShotHUD(TacticalHUD.m_kShotHUD).RemoveAll().InitLayout();
